...
async function main() {
  ...
  if (!inputPath || !outputPath) {
    throw new Error("Usage: node tools/build-hiroshima-boundaries.mjs <input.geojson> <output.geojson>");
  }
  ...
  const transformed = {
    ...
    name: "hiroshima_city_boundaries",
    crs_note: "Converted to EPSG:4326 from e-Stat GeoPackage source.",
  ...
  if (jsOutputPath !== outputPath) {
    await fs.writeFile(jsOutputPath, `window.HIROSHIMA_CITY_BOUNDARIES = ${serialized};\n`);
  }
