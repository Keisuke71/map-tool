...
async function main() {
  ...
  if (!inputPath || !outputPath) {
    throw new Error("Usage: node tools/build-kyoto-boundaries.mjs <input.geojson> <output.geojson>");
  }
  ...
  const transformed = {
    ...
    name: "kyoto_city_boundaries",
    crs_note: "Source GeoPackage coordinates are JGD2000 (EPSG:4612) longitude/latitude and are compatible with EPSG:4326 display.",
  ...
  if (jsOutputPath !== outputPath) {
    await fs.writeFile(jsOutputPath, `window.KYOTO_CITY_BOUNDARIES = ${serialized};\n`);
  }
